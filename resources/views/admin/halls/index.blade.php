        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            @forelse($halls as $hall)
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ $hall->capacity }} asientos</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                {{ $hall->type == 'imax' ? 'bg-purple-100 text-purple-800' :
                                   ($hall->type == '4dx' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800') }}">
                                {{ strtoupper($hall->type) }}
                            </span>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $hall->features ?? 'Estándar' }}</td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <form action="{{ route('admin.halls.toggle-status', $hall) }}" method="POST" class="inline">
                            @csrf
                            @method('PUT')
                            <button type="submit"
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $hall->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $hall->is_active ? 'Activa' : 'Inactiva' }}
                            </button>
                        </form>
                    </td>
